RSpec.describe "Books", type: :request do
  before(:each) do
    allow_any_instance_of(BooksController).
  ...
  describe "GET /api/books" do
    ...
    describe "field picking" do
      context "with the fields parameter" do
        before(:each) { get "/api/books?fields=id,title,author_id" }
      ...
      context 'with invalid field name "fid"' do
        before(:each) { get "/api/books?fields=fid,title,author_id" }
    ...
    describe "pagination" do
      context "when asking for the first page" do
        before(:each) { get("/api/books?page=1&per=2") }
      ...
      context "when asking for the second page" do
        before(:each) { get("/api/books?page=2&per=2") }
      ...
      context "when sending invalid 'page' and 'per' parameters" do
        before(:each) { get("/api/books?page=fake&per=25") }
    ...
    describe "sorting" do
      ...
      context "with invalid column name 'fid'" do
        before(:each) { get "/api/books?sort=fid&dir=asc" }
  ...
  describe "GET /api/books/:id" do
    context "with existing resource" do
      before(:each) { get "/api/books/#{rails_tutorial.id}" }
    ...
    context "with nonexistant resource" do
      it "returns HTTP status 404" do
        get "/api/books/1234321"
  ...
  describe "POST /api/books" do
    ...
    before(:each) { post "/api/books", params: { data: params } }
    ...
    context "with invalid parameters" do
      ...
      it "receives an error details" do
        expect(json_body["error"]["invalid_params"]).to eq("title" => ["can't be blank"], "author" => ["can't be blank"])
      end
