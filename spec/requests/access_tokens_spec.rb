RSpec.describe "Access Tokens", type: :request do
  ...
  describe "POST /api/access_tokens" do
    context "with valid API key" do
      ...
      let(:headers) do
        { "HTTP_AUTHORIZATION" => "Codices-Token api_key=#{key.id}:#{key.key}" }
      end
  ...
  describe "DELETE /api/access_tokens" do
    context "with valid API key" do
      ...
      context "with valid access token" do
        let(:access_token) do
          create(:access_token, api_key: api_key, user: john)
        end

        let(:token) { access_token.generate_token }
        let(:token_str) { "#{john.id}:#{token}" }
        let(:headers) do
          token = { "HTTP_AUTHORIZATION" => "Codices-Token api_key=#{api_key_str}, access_token=#{token_str}" }
        end
      ...
      context "with invalid access token" do
        let(:headers) do
          { "HTTP_AUTHORIZATION" => "Codices-Token api_key=#{api_key_str}, access_token=1:fake" }
        end
