RSpec.describe "User Auth Flow", type: :request do
  def headers(user_id = nil, token = nil)
    ...
    if user_id && token
      ...
      { "HTTP_AUTHORIZATION" => "Codices-Token api_key=#{api_key_str}, access_token=#{token_str}" }
    else
  ...
  it "authenticate a new user" do
    ...
    patch "/api/users/#{id}", params: { data: { given_name: "John" } }, headers: headers(user_id, token)
